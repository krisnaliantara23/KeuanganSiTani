...
export async function getPendapatan(token) {
  const res = await axios.get(`${API_URL}/keuangan/laporan?jenis=pemasukan`, getAuthHeader(token));
  return res.data.data;
}
...
export async function addPendapatan(token, data) {
  ...
  const res = await axios.post(`${API_URL}/keuangan/laporan`, payload, getAuthHeader(token));
  return res.data.data;
}

// --- Pengeluaran ---
export async function getPengeluaran(token) {
  const res = await axios.get(`${API_URL}/keuangan/laporan?jenis=pengeluaran`, getAuthHeader(token));
  return res.data.data;
}
...
export async function getNeracaSummary(token) {
  const res = await axios.get(`${API_URL}/neraca/summary`, getAuthHeader(token));
  return res.data;
}
